...
2. **Submission is Code-Competition notebook rerun, not file upload —
   ...
   first assumed (see "Kaggle Access Troubleshooting" below).
...
See the deviation above — this is a Code Competition (notebook rerun,
submitted via `KaggleApi.competition_submit_code(...)`), matching rather
than deviating from the shared `coding-standards/coding_standards.md` (§11)
