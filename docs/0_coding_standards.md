...
2. **Submission is Code-Competition notebook rerun, not file upload —
   ...
   `docs/1_instructions.md` for the confirmed round trip (a manual
   "Submit to Competition" click on a completed Kaggle kernel run — no
   CLI/API path exists for it).
...
See the deviation above — this is a Code Competition (notebook rerun via
the web UI's "Submit to Competition" button), matching rather than
deviating from the shared `coding-standards/coding_standards.md` (§11)
preference for notebook-based submission. Refer to `docs/1_instructions.md`
for the confirmed round trip.
...
No `kaggle` CLI/API command exists to submit a kernel's output directly;
the only path is the Kaggle web UI's "Submit to Competition" button on a
`COMPLETE` kernel run (the kernel needs `competition_sources` set in its
`kernel-metadata.json` for that button to appear at all). Generalizes: a
competition baseline's own docs about the submission mechanism aren't
authoritative — verify with a real API call before assuming either
direction (file-upload vs. notebook-rerun).
